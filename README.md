...
3. `cp deploy/.env.production.example deploy/.env.production` and set `DJANGO_SECRET_KEY`, `DEEPSEEK_API_KEY` (only on the server file — never in git), and hosts/CORS values.
...
3. On the server, create `deploy/.env.production` once (see `deploy/.env.production.example`) with `DJANGO_SECRET_KEY`, `DEEPSEEK_API_KEY`, etc.
