    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-slate-900/50">
      <div className="w-full sm:max-w-2xl max-h-[92vh] overflow-hidden bg-white rounded-t-3xl sm:rounded-3xl shadow-2xl flex flex-col">
        <div className="px-5 py-4 border-b border-slate-100 flex items-start justify-between gap-3">
          <div>
            <h3 className="text-lg font-bold text-slate-900">Tahlil natijasi — tekshiring</h3>
            <p className="text-[12px] text-slate-500 mt-0.5 leading-relaxed">
              AI hujjatdan fan nomi va mavzularni ajratdi. Noto‘g‘ri bo‘lsa, tahrirlang yoki bekor qiling.
            </p>
          </div>
          <button
            type="button"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-2">
            <StatCard label="Jami mavzu" value={String(totalTopics)} />
            <StatCard label="Ma'ruza" value={String(totalLectures)} icon={<BookOpen size={14} />} />
            <StatCard label="Amaliy" value={String(totalPracticals)} icon={<FlaskConical size={14} />} />
          </div>

          <div className="rounded-xl border border-indigo-100 bg-indigo-50/50 px-3 py-2 text-[11px] text-indigo-900 leading-relaxed">
            <strong>Fan</strong> — bitta o‘quv predmeti (masalan Anatomiya).
            <br />
            <strong>Yo‘nalish</strong> — dastur turi (PI, DI); fayl nomidagi qavsdan olinadi.
            <br />
            <strong>Mavzu</strong> — dars rejadagi har bir band (M1, L2, A1…).
          </div>
              ...
              >
                <div className="bg-slate-50 px-3 py-2 flex flex-wrap items-center gap-2">
                  <label className="flex items-center gap-2 flex-1 min-w-[140px]">
                    <span className="text-[10px] font-bold text-slate-500 uppercase">Yo‘nalish</span>
                    <input
                ...
                <div className="px-3 py-2 text-[11px] text-slate-600 border-b border-slate-100">
                  {variant.topics.length} mavzu · {counts.lectures} ma’ruza · {counts.practicals} amaliy
                </div>
                <ul className="max-h-40 overflow-y-auto divide-y divide-slate-50">
                  ...
                  {variant.topics.length > 24 && (
                    <li className="px-3 py-2 text-[11px] text-slate-400 italic">
                      + yana {variant.topics.length - 24} ta mavzu…
                    </li>
                  )}
                </ul>
              </div>
